...
def parse_pcap(path, delta_t):
    ...
    pcap1_path_pattern = os.path.join(path, PCAP1 + '*')
    pcap2_path_pattern = os.path.join(path, PCAP2 + '*')

    pcap1_files = glob.glob(pcap1_path_pattern)
    pcap2_files = glob.glob(pcap2_path_pattern)

    # Check if the pcap files were found before trying to access them
    if not pcap1_files:
        print(colorize(f"Error: No PCAP file found matching the pattern '{pcap1_path_pattern}'", 'red'))
        sys.exit(1)  # Exit the script with an error code

    if not pcap2_files:
        print(colorize(f"Error: No PCAP file found matching the pattern '{pcap2_path_pattern}'", 'red'))
        sys.exit(1)
    ...
    for ts, buf in pcap:
        ...
        try:
            ...
            for opt in options:
                if opt[0] == dpkt.tcp.TCP_OPT_TIMESTAMP:
                    # Using struct to unpack bytes safely
                    ts_val, ts_ecr = struct.unpack('!II', opt[1])
        except (struct.error, dpkt.dpkt.UnpackError):
            # Handle cases where TCP options are malformed or missing
        ...
        if src_port > dst_port:
            ...
            if ts_ecr is not None and ts_ecr in ts_vals[connection_index][1]:
    ...
    for ts, buf in pcap:
        ...
        if tcp_tuple not in connections:
            continue
